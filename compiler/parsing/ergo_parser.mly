...
let relative_name_of_qname qn =
  begin match qn with
  | (None,last) -> (None,char_list_of_string last)
  | (Some prefix, last) ->
      (Some (char_list_of_string prefix),
...
let make_template_ulist prov name ve =
  let a = char_list_of_string name in
...
let make_template_join prov name sep ve =
  ...
  ErgoCompiler.ebinarybuiltin prov
    ErgoCompiler.ErgoOps.Binary.opstringjoin
    (ErgoCompiler.econst prov (ErgoCompiler.ErgoData.dstring (char_list_of_string sep)))
    (ErgoCompiler.eforeach prov fl None
...
let make_template_if prov name veTrue =
  let veFalse = [ErgoCompiler.econst prov (ErgoCompiler.ErgoData.dstring (char_list_of_string ""))] in
...
| NAMESPACE qn = qname_prefix ds = decls
    { { module_annot = mk_provenance $startpos $endpos;
        module_file = char_list_of_string !filename;
        module_prefix = char_list_of_string (class_prefix_of_filename !filename);
        module_namespace = qn;
...
param:
| pn = IDENT
    { (char_list_of_string pn, ErgoCompiler.ergo_type_any (mk_provenance $startpos $endpos)) }
| pn = IDENT COLON pt = paramtype
    { (char_list_of_string pn, pt) }
...
| CALL cln = IDENT LPAREN el = exprlist RPAREN
    { let e0 = ErgoCompiler.ethis_contract (mk_provenance $startpos $endpos) in
      ErgoCompiler.scallclause (mk_provenance $startpos $endpos) e0 (char_list_of_string cln) el }
| SEND e1 = expr
    { let e0 = ErgoCompiler.ethis_contract (mk_provenance $startpos $endpos) in
...
| i = INT
    { ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dnat (coq_Z_of_int i)) }
| f = FLOAT
    { ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dfloat f) }
| s = STRING
    { ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s)) }
| LBRACKET el = exprlist RBRACKET
    { ErgoCompiler.earray (mk_provenance $startpos $endpos) el }
(* Text *)
| OPENTEXT sl = textlist s0 = CLOSETEXT
    { let slast = ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s0)) in
      let sl' = sl @ [slast] in
...
| s0 = OPENEXPR e = expr CLOSEEXPR sl = textlist
    { let sfirst = ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s0)) in
...
| s0 = OPENVARSHARP CLAUSE v0 = safeblock_base CLOSEVAR sl1 = textlist s1 = OPENVARSLASH CLAUSE CLOSEVAR sl2 = textlist
    { begin
        let sfirst = ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s0)) in
        let slast = ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s1)) in
...
| s0 = OPENVARSHARP IF v0 = safeblock_base CLOSEVAR sl1 = textlist s1 = OPENVARELSE CLOSEVAR sl2 = textlist s2 = OPENVARSLASH IF CLOSEVAR sl3 = textlist
    { begin
        let sfirst = ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s0)) in
        let smid = ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s1)) in
        let slast = ErgoCompiler.econst (mk_provenance $startpos $endpos) (ErgoCompiler.ErgoData.dstring (char_list_of_string s2)) in
...
| qn = qname_base
    { begin match qn with
      ...
      | (None,last) -> char_list_of_string last
      | (Some prefix, last) -> char_list_of_string (prefix ^ "." ^ last)
      end }
...
data:
| s = STRING
    { ErgoCompiler.ErgoData.dstring (char_list_of_string s) }
| i = INT
