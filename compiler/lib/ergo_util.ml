...
let anon_args cto_files input_files template_files f =
  let extension = Filename.extension f in
  if extension = ".ctoj"
...
let parse_args args_list usage args gconf =
  ...
  let template_files = ref [] in
  parse args (args_list gconf) (anon_args cto_files input_files template_files) usage;
  (List.rev !cto_files, List.rev !input_files, List.rev !template_files)
